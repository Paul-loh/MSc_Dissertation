CREATE PROCEDURE [etl].[INITIAL_LOAD_Import_Transactions_From_Integration_DB]	(
																					@LoadDateTime	DATETIME2
																				)
-- WITH EXECUTE AS OWNER 
AS

--	TBD!!!				This should be replaced by an import from a full history Pacer Transactions file.

--	Author:				Paul Loh	
--	Creation Date:		20200203
--	Description:		DWH Initial Load 
--						
--						Import all Transactions from the Integration DB.
-- 
--						Parameters:
...
--			
	...
	FROM		[.].[CAL_JBETL].inv.Transactions t
	left join	[.].[CAL_JBETL].inv.Currencies c1 on t.PriceCcy		 = c1.CurrencyCode
	left join	[.].[CAL_JBETL].inv.Currencies c2 on t.SettleCcy	 = c2.CurrencyCode
	;
