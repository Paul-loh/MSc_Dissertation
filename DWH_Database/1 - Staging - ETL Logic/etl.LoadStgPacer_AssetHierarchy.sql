CREATE PROCEDURE [etl].[PACER_LoadStgAssetHierarchy] (
...
AS
	/*
			Author:			Paul Loh
			Date:			2019-12-12
			Description:	Load Pacer Asset Hierarchy master data to Staging Layer
			Changes:		
	*/	
...
BEGIN
	...
	BEGIN TRY
		...
		UPDATE stg.PACER_AssetHierarchy
			...
			-- Parent Asset Code and Asset Code 
				[Out_HKeyLinkAssetHierarchy]		=	HASHBYTES	( N'SHA1',																		
																		CONCAT_WS( N'|',
																						COALESCE( [Out_ParentCode], N'')	-- Asset Parent Code
																						,COALESCE( [Out_Code], N'')			-- Asset Child Code
																				)																			
																	)																			
																	
			-- Asset Parent Code
				-- It's valid for some records not to have Parent codes as they are at the 'top of the hierarchy'
				, Out_HKeyAssetParentCode			=	CASE	WHEN [Out_ParentCode] IS NULL 
	...
	BEGIN CATCH
		...
		RAISERROR(N'etl.PACER_LoadstgAssetHierarchy : %d : %s', 16, 1, @error, @message);
		RETURN (-1);
